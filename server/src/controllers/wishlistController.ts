...
export async function toggle(req: Request, res: Response, next: NextFunction) {
  try {
    const result = await wishlistService.toggleWishlist(req.user!.id, req.params.paintingId);
...
export async function remove(req: Request, res: Response, next: NextFunction) {
  try {
    await wishlistService.removeFromWishlist(req.user!.id, req.params.paintingId);
